...
define config.name = _("Сражение в виртуальной паутине")
...
define gui.about = _p(""" Жанр: Приключенческая новелла с элементами научной фантастики

Аннотация: 

"Сражение в Виртуальной паутине" - это захватывающая визуальная новелла, которая рассказывает историю девушки, стоящей перед выбором между backend и frontend разработкой. Она мечтает стать веб-разработчиком, но сталкивается с сомнениями и давлением окружающих, которые советуют ей искать другую профессию.

Игрок погружается в эмоциональный мир героини, влияет на ход сюжета и развивает своего персонажа, узнает больше о веб-разработке и преодолевает сомнения вместе с героиней. Однако, судьбоносный день переворачивает все сомнения с ног на голову...

"Сражение в Виртуальной паутине" - это захватывающая и волнующая история о силе веры в себя и важности преследования своих мечт. Погрузитесь в мир разработчика, узнайте больше о backend и frontend разработке и откройте для себя новые пути к успеху.

Создатели: Команда \" Арака\"
""")
...
define build.name = "battle_in_the_virtual_web"
...
define config.main_menu_music = "start_music.ogg"
...
define config.save_directory = "TheCodeQueen-1699540433"
